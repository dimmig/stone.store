export const DEFAULT_MENU_COLOR = "#9DB2BF";
export const DEFAULT_TEXT_COLOR = "#ffffff";
export const GRAY_TEXT_COLOR = "#a8a8a8";

export function drawColors(item, w, h) {
...
export function generateTints(hexColor, numTints) {
  ...
  const tints = [];
  const lightnessStep = 1 / numTints;

  for (let i = 1; i <= numTints; i++) {
    hslColor[2] = Math.min(hslColor[2] + lightnessStep, 1);
    const rgbColor = hslToRgb(hslColor[0], hslColor[1], hslColor[2]);
    const hexTint = rgbToHex(rgbColor[0], rgbColor[1], rgbColor[2]);
    tints.push(hexTint);
  }
